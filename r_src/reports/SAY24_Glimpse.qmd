...
In short, methods such as Entropy, Rank, Log-Rank, and Fast-DetectGPT are passed to proprietary models in order to detect the probability that the LLM itself would generate a given response (Bao, et al. 2025).

## Methodology

All modifications to the base Glimpse package have been related to input and output of large datasets. The base Glimpse model has not been altered and a full explanation of the Glimpse methodology can be found in Bao, et al. 2025.
...
Looking at the distribution of `final_score` we see and average of .30 (30%) chance of machine generation across all participants that responded at least once. 99.5% of all participants have a less than 75% chance of being machine generated, indicating a low incidence rate of machine generation responses during this wave. Considering the possibility that some participants are well articulated\* enough to create false positives for this approach, we can infer that we are within the bounds of low fraud risk for this wave.

\* The model is based on tokenization probabilities, not the grammar used by the respondent. There is a correlation between these
...
This table contains the first 50 rows of the final output table (n = 12288 responses).
